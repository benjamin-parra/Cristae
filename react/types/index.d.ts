...
// ── Hits de picking ─────────────────────────────────────────────────────────
// Los emite el bus del motor, así que los define el núcleo: acá sólo se les da el nombre con que el
// binding los expone. La lista llega ordenada top-first (`zIndex` desc, `order` asc, `distancePx`
// asc), así que el consumidor desambigua con `hits[0]` sin recalcular geometría.
...
// La etiqueta y su painter los define el núcleo; acá sólo se les da el nombre con que el binding los
// expone. Alias y no copias: así `paint={drawLabel}` calza y no pueden divergir.
...
// Los payloads de los canales `cluster:*` los construye el fold del núcleo; acá sólo se les da el
// nombre con que el binding los expone.

export type CristaeClusterEntity<T = unknown>  = ClusterEntity<T>
export type CristaeClusterGroup<T = unknown>   = ClusterGroup<T>
export type CristaeClusterSession<T = unknown> = ClusterSession<T>
export type CristaeClusterDismiss              = ClusterDismiss
export type CristaeClusterMarked               = ClusterMarked
...
/** El `<cristae-overlay>` vivo, por `ref`. Es un MODIFICADOR: no da de alta una capa propia (la crea
 *  la gramática), así que no expone `controls` — lo que sí admite es reasignar sus props en caliente,
 *  para re-evaluar los badges sin pasar por un render de React. */
export interface CristaeOverlayElement<T = unknown> extends HTMLElement {
  iconSet   : IconSet | string;
  variantOf : (item: T) => string;
  sizeOf    : (item: T) => number;
  where     : (item: T) => boolean;
